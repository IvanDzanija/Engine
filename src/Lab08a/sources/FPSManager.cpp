...
FPSManager::FPSManager(GLFWwindow *window, int theTargetFps, float theReportInterval,
                       std::string theWindowTitle) {
  init(window, theTargetFps, true);  // If you specify a window title it's safe to say
                                     // you want the FPS to update there ;)
...
bool FPSManager::getVerbose() { return verbose; }

void FPSManager::setVerbose(bool theVerboseValue) { verbose = theVerboseValue; }

int FPSManager::getTargetFps() { return targetFps; }
...
void FPSManager::setTargetFps(int theFpsLimit) {
  ...
  if (theFpsLimit < MIN_TARGET_FPS) {
    ...
    std::cout << "Limiting FPS rate to legal minimum of " << MIN_TARGET_FPS
              << " frames per second." << std::endl;
  }
  if (theFpsLimit > MAX_TARGET_FPS) {
    theFpsLimit = MAX_TARGET_FPS;
    std::cout << "Limiting FPS rate to legal maximum of " << MAX_TARGET_FPS
              << " frames per second." << std::endl;
  }
...
double FPSManager::getFrameDuration() { return frameDuration; }

void FPSManager::setReportInterval(float theReportInterval) {
  // Ensure the time interval between FPS checks is sane (low cap = 0.1s, high-cap
  // = 10.0s) Negative numbers are invalid, 10 fps checks per second at most, 1 every 10
  // secs at least.
...
double FPSManager::enforceFPS(bool shouldSleep) {
  ...
  // Calculate how long it's been since the frameStartTime was set (at the end of this
  // method)
  ...
  if (reportInterval != 0.0f) {
    ...
    if ((frameEndTime - lastReportTime) > reportInterval) {
      ...
      // Reset the frame counter to 1 (and not zero - which would make our FPS values
      // off)
      frameCount = 1;
    ...
    } else  // FPS calculation time interval hasn't elapsed yet? Simply increment the
            // FPS frame counter
    {
      ++frameCount;
    }
  ...
  if (shouldSleep)
    if (sleepDuration > 0.0) {
      std::this_thread::sleep_for(std::chrono::microseconds(
          (int)(1000000 * (targetFrameDuration - frameDuration))));
    }
  // Reset the frame start time to be now - this means we only need put a single call
  // into the main loop
  frameStartTime = glfwGetTime();

  // Pass back our total frame duration (including any sleep and the time it took to run
  // this function) to be used as our deltaTime value
  return frameDuration + (frameStartTime - frameEndTime);

}  // End of our enforceFPS method

int FPSManager::getFrameCount() { return totalFrameCount; }
