...
// Jedno od mogucih rjesenja je da se zakomentira linija #define
...
int main(int argc, char *argv[]) {
  ...
  const aiScene *scene = importer.ReadFile(
      objPath.c_str(), aiProcess_CalcTangentSpace | aiProcess_Triangulate |
  ...
  if (scene->HasMeshes()) {
    ...
    for (int i = 0; i < 10 && i < mesh->mNumFaces; i++) {
      ...
      // OPREZ! razmisliti prije nego se kopiraju dijelovi ovog koda. OpenGL želi
      // indeksirane vrhove, normale i uv koordinate, a tu smo prikazali te podatke
      // slijedno, a ne indeksirano!

      for (int j = 0; j < mesh->mFaces[i].mNumIndices; j++) {
        int vertex =
            mesh->mFaces[i].mIndices[j];  // OPREZ! grafickoj kartici cete po uputama
                                          // slati indeksirane buffere, a ne slijedne
        std::cout << "   coordinates    xyz  " << mesh->mVertices[vertex].x << " "
                  << mesh->mVertices[vertex].y << " " << mesh->mVertices[vertex].z
                  << std::endl;
      }

      std::cout << std::endl;

      // uz svaku tocku na poligonu je pridruzena i uv koordinata na teksturi preko
      // istog indeksa
      ...
      for (int j = 0; j < mesh->mFaces[i].mNumIndices; j++) {
        int vertex =
            mesh->mFaces[i].mIndices[j];  // OPREZ! grafickoj kartici cete po uputama
                                          // slati indeksirane buffere, a ne slijedne
        std::cout << "   texture         uv  " << mesh->mTextureCoords[0][vertex].x
                  << " " << mesh->mTextureCoords[0][vertex].y << std::endl;
      }
      ...
      for (int j = 0; j < mesh->mFaces[i].mNumIndices; j++) {
        int vertex =
            mesh->mFaces[i].mIndices[j];  // OPREZ! grafickoj kartici cete po uputama
                                          // slati indeksirane buffere, a ne slijedne
        std::cout << "   normals        xyz  " << mesh->mNormals[vertex].x << " "
                  << mesh->mNormals[vertex].y << " " << mesh->mNormals[vertex].z
                  << std::endl;
      }
  ...
  if (scene->HasMaterials()) {
    ...
    for (int i = 0; i < scene->mNumMaterials; i++) {
      ...
      unsigned char *data;
      if (AI_SUCCESS ==
          scene->mMaterials[i]->Get(AI_MATKEY_TEXTURE(aiTextureType_DIFFUSE, 0),
                                    texturePosition)) {
        ...
        // stbi_set_flip_vertically_on_load(true); //flipana y koordinata kod robota,
        // obavezno provjeriti
