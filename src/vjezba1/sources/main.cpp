...
void klikMisa(int x, int y, int vrsta) {
  if (vrsta == 0) std::cout << "lijevi:";
  if (vrsta == 1) std::cout << "desni:";

  std::cout << x << " " << y << std::endl;
  klikovi.emplace_back(x, y);
}
...
int main(int argc, char *argv[]) {
  ...
  // prosljeduje se pokazivac na funkciju. Ta funkcija ce se pozvati kada korisnik
  // pritisne misem
  ...
  while (grafika.trebaZatvoriti()) {
    ...
    for (int i = 0; i < height; i += 1)
      for (int j = 0; j < width; j += 1) {
        if ((i + j) % 2 == 0) grafika.osvijetliFragment(i, j, glm::vec3(0.1, 0.1, 0.1));
        if (i % 10 == 0 && j % 10 == 0)
    ...
    for (auto &klik : klikovi) {
      grafika.osvijetliFragment(klik.first, height - klik.second - 1,
                                glm::vec3(0.6, 0.2, 0));
    }
