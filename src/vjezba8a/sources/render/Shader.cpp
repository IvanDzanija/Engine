...
Shader::Shader(const std::string &vertex_name, const std::string &fragment_name,
  ...
  const std::string vertex_path = path + vertex_name + ".vert";
  const std::string fragment_path = path + fragment_name + ".frag";
  const std::string geometry_path = path + geometry_name + ".geom";
  uint32 vertex;
  uint32 fragment;
  uint32 geometry;
  ...
  if (!geometry_name.empty()) {
    try {
      std::string geometry_code = _read_shader_source(geometry_path.c_str());
      geometry = _compile_shader(geometry_code, GL_GEOMETRY_SHADER);
