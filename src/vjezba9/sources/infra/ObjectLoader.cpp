...
std::string sanitize_path(const std::string &path) {
  ...
  size_t pos;
  while ((pos = clean_path.find("\\\\")) != std::string::npos) {
    clean_path.replace(pos, 2, "/");
  }
  while ((pos = clean_path.find('\\')) != std::string::npos) {
    clean_path.replace(pos, 1, "/");
  }
  return clean_path;
}
}  // namespace detail
...
// METHODS
// ----------------------------------
Model ObjectLoader::load_model(const std::string &directory, const std::string &name) {
  std::string path =
      std::filesystem::current_path().string() + RESOURCES_DIR + directory + "/" + name;
  ...
  _process_node(scene->mRootNode, scene, directory, meshes);
...
void ObjectLoader::_process_node(aiNode *node, const aiScene *scene,
  ...
  for (size_t i = 0; i < node->mNumMeshes; ++i) {
    ...
    meshes.emplace_back(std::make_shared<Mesh>(_process_mesh(mesh, scene, directory)));
  }
  for (size_t i = 0; i < node->mNumChildren; ++i) {
    _process_node(node->mChildren[i], scene, directory, meshes);
  }
}
...
Mesh ObjectLoader::_process_mesh(aiMesh *mesh, const aiScene *scene,
  ...
  std::vector<std::shared_ptr<Texture>> diffuse_maps =
      _process_textures(material, aiTextureType_DIFFUSE, "diffuse", directory);
  textures.insert(textures.end(), diffuse_maps.begin(), diffuse_maps.end());
  // 2. specular maps
  std::vector<std::shared_ptr<Texture>> specular_maps =
      _process_textures(material, aiTextureType_SPECULAR, "specular", directory);
  textures.insert(textures.end(), specular_maps.begin(), specular_maps.end());
  // 3. normal maps
  std::vector<std::shared_ptr<Texture>> normal_maps =
      _process_textures(material, aiTextureType_HEIGHT, "normal", directory);
  textures.insert(textures.end(), normal_maps.begin(), normal_maps.end());
  // 4. height maps
  std::vector<std::shared_ptr<Texture>> height_maps =
      _process_textures(material, aiTextureType_AMBIENT, "height", directory);
  textures.insert(textures.end(), height_maps.begin(), height_maps.end());
  std::cout << textures.size() << " textures loaded for mesh " << mesh->mName.C_Str()
            << std::endl;

  return {vertices, indices, textures, material_index};
}
std::vector<std::shared_ptr<Texture>> ObjectLoader::_process_textures(
  ...
  for (size_t i = 0; i < material->GetTextureCount(type); ++i) {
    ...
    std::string relative_path = detail::sanitize_path(texture_path.C_Str());

    std::string path = std::filesystem::current_path().string() + RESOURCES_DIR +
                       directory + "/" + detail::sanitize_path(texture_path.C_Str());

    std::cout << "Processing texture: " << path << std::endl;

    std::shared_ptr<Texture> texture = ResourceManager::get_texture(type_name, path);

    if (texture == nullptr) {
      texture = std::make_shared<Texture>(type_name, path);
      ResourceManager::add_texture(type_name, path, texture);
    }
